<?php 
	...
	if(isset($_POST['btn_novoAluno'])){
		...
		$novoAluno = $con->prepare("INSERT INTO tb_usuarios(matricula, nome, senha, email, id_turma, aluno) VALUES(?, ?, ?, ?, ?, true)");
		$novoAluno->bindParam(1, $matriculaAluno);
		$novoAluno->bindParam(2, $nomeAluno);
		$novoAluno->bindParam(3, sha1($senhaAluno));
		...
		if($novoAluno->execute()){
			$mensagem = "
				...
				<b>Link para o site:</b> <a href='#link'>Clique aqui</a><br>
			...
			echo "<script>alert('Aluno cadastrado com sucesso\nTodos os dados de login do aluno foi enviado para o email dele'); </script>";
		}
	}
